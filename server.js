...
app.get('/api/matchmaking/status/:matchKey/:walletAddress', (req, res) => {
    ...
    if (matchedPlayers.has(walletAddress)) {
        // --- FINAL LOGIC FIX ---
        // Player was matched. Find their opponent and clean up BOTH entries.
        const opponentAddress = matchedPlayers.get(walletAddress);

        matchedPlayers.delete(walletAddress);
        if (opponentAddress) {
            matchedPlayers.delete(opponentAddress);
        }
        
        console.log(`[STATUS] Confirmed match for ${walletAddress}. Cleaning up match entries.`);
