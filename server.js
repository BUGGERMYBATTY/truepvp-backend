...
const { v4: uuidv4 } = require('uuid'); // Use a more robust ID generator if available
    ...
    gameState.players.forEach(player => {
        if (player.ws.readyState === WebSocket.OPEN) {
            ...
            const stateForPlayer = {
                ...gameState,
                you: player,
                opponent: gameState.players.find(p => p.walletAddress !== player.walletAddress),
                players: undefined, // Don't send the full players array with WebSocket objects
            };
...
function processRound(gameId) {
    ...
    if (roundWinner) {
        ...
        gameState.roundMessage = `${roundWinner.walletAddress === p1.walletAddress ? 'Player 1' : 'Player 2'} wins ${points} points!`;
    } else {
    ...
    setTimeout(() => {
        if (gameState.round >= 5) {
            ...
            gameState.roundMessage = winner ? `Game Over! Winner is ${winner.walletAddress === p1.walletAddress ? 'Player 1' : 'Player 2'}` : 'Game Over! It is a draw!';
